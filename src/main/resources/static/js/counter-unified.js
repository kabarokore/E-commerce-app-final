...
(function () {
  ...
  const core = {
    init: function () {
      ...
      state.cartCount = utils.storage.get("cartCount", 0);
      state.wishlistCount = utils.storage.get("wishlistCount", 0);

      // Update DOM counters with cached values first (safely)
      const safeCartCount = parseInt(state.cartCount || 0, 10);
      const safeWishlistCount = parseInt(state.wishlistCount || 0, 10);
      ...
      window.addEventListener("storage", function (e) {
        try {
          if (e.key === "cartCount") {
            ...
            const count = parseInt(data.value || 0, 10);
            state.cartCount = count;
    ...
    refreshCounts: async function () {
      ...
      try {
        const cartCount = await api.fetchCartCount();
      ...
      try {
        const wishlistCount = await api.fetchWishlistCount();
        if (
